...
Replace `your_secure_password_here` with the password you made for Redis. Plex uses Redis to pass admin-chat messages between servers and to tell the other servers when a punishment changes. Plex stores player data, punishments, and indefinite bans outside Redis.
