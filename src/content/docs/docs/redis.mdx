...
Replace `your_secure_password_here` with the password you made for Redis. Plex uses Redis for indefinite bans and for passing punishment and admin-chat messages between servers on your network. Plex stores player data and punishments in the SQL database, not in Redis.
