...
A banned player may still join in spectator mode when a spectator slot is free. Plex kicks that player with
...
Plex uses Oasis for a rollback. If Oasis is not installed, Plex uses CoreProtect. Without either plugin, no rollback
runs.
